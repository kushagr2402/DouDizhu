...
(define (bombs hand)
  (cond [(or (empty? hand) (empty? (rest hand))
         ...
         empty]
        [(and (equal? (first hand) (first (rest hand)))
              (equal? (first hand) (first (rest (rest hand))))
              (equal? (first hand) (first (rest (rest (rest hand))))))
...
(define (rockets hand)
  (cond [(or (empty? hand) (empty? (rest hand))) empty]
        [(and (equal? 'Black (first hand)) (equal? 'Red (second hand)))
         '((Black Red))]
...
(define (student holding role played)
  (cond
    [(both-passed played)
     (cond
       [(not (empty? (airplanes holding))) (first (airplanes holding))]
       [(not (empty? (straights holding))) (first (straights holding))]
       [(not (empty? (straight-pairs holding))) (first (straight-pairs holding))]
       [(not (empty? (pairs holding))) (last (pairs holding))]
       [(not (empty? (bombs holding))) (first (bombs holding))]
       [(not (empty? (trios holding))) (last (trios holding))]
       [(not (empty? (rockets holding))) (first (rockets holding))]
       [else (list (first holding))])]
    [(symbol=? role 'Landlord)
     (local [(define could-play (follow (hand-to-beat played) holding))]
       (cond
         [(empty? could-play) empty]
         [(and (not (empty? (straights holding))) (member? (last (straights holding)) could-play))
          (last (straights holding))]        
         [(and (not (empty? (pairs holding))) (member? (first (pairs holding)) could-play))
          (first (pairs holding))]
         [(and (not (empty? (bombs holding))) (member? (last (bombs holding)) could-play))
          (last (bombs holding))]
         [(and (not (empty? (trios holding))) (member? (first (trios holding)) could-play))
          (first (trios holding))]
         [(and (not (empty? (airplanes holding))) (member? (first (airplanes holding)) could-play))
          (first (airplanes holding))]
         [(and (not (empty? (straight-pairs holding)))
               (member? (first (straight-pairs holding)) could-play))
          (first (straight-pairs holding))]
         [(and (not (empty? (rockets holding))) (member? (first (rockets holding)) could-play))
          (first (rockets holding))]
    ...
    [(symbol=? role 'Right)
     (local [(define could-play (follow (hand-to-beat played) holding))]
       (cond [(empty? could-play) empty]
             [(and (not (empty? (rockets holding))) (member? (first (rockets holding)) could-play))
              (first (rockets holding))]
             [(and (not (empty? (bombs holding))) (member? (first (bombs holding)) could-play))
              (first (bombs holding))]
             [else (first could-play)]))]
    [else
     (reckless holding role played)]))
